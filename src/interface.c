...
void part(sfRenderWindow *window, ois_t *oiseaux)
{
    int random = rand() % 900;
...
void retry(sfRenderWindow *window,ois_t *oiseaux, sfIntRect *box,sfEvent event)
{
    int random = rand() % 900;
...
void clock(sfRenderWindow *window, ois_t *oiseaux, menu_t *menu, heart_t *heart)
{
...
void interface(sfRenderWindow* window, sfEvent event)
{
    ...
    while (sfRenderWindow_isOpen(window)) {
        ...
        retry(window , oiseaux, &heart->box, event);
        if (deltatime >= 0.2) {
            move(&oiseaux->hitbox);
            sfClock_restart(horloge);
        }
    sfSprite_move(oiseaux->sprite, V2F(0.3, 0.0));
    clock(window, oiseaux, menu, heart);
    }
}
